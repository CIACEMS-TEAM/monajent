...
const navItems = [
  ...
  // { to: '/agent/wallet', icon: 'wallet', label: 'Revenus' },  // masqué — freemium
  ...
  >
    ...
    <header class="agt-header">
      ...
      <div class="agt-header__end">
        <button
          class="agt-header__mobile-search"
        ...
        <router-link
          to="/agent/listings?action=new"
          class="agt-header__create"
        ...
        <div class="agt-notif-wrapper">
          <button class="agt-header__bell" @click.stop="toggleNotif" title="Notifications">
            ...
            <span v-if="notifStore.unreadCount > 0" class="agt-header__bell-badge">{{
          ...
          <div v-if="notifOpen" class="agt-notif-panel">
            <div class="agt-notif-panel__header">
              ...
              <button
                ...
                class="agt-notif-panel__mark-all"
                @click="markAllRead"
              >
                Tout lire
              </button>
            ...
            <div v-else-if="notifStore.notifications.length === 0" class="agt-notif-panel__empty">
              Aucune notification
            </div>
            <div v-else class="agt-notif-panel__list">
              ...
              >
                <div class="agt-notif-item__icon" :class="'cat-' + n.category.toLowerCase()">
                  <svg v-if="n.category === 'KYC'" viewBox="0 0 24 24" width="18" height="18">
                    <path
                      fill="currentColor"
        ...
        <div class="agt-profile-menu">
          <div class="agt-avatar-wrap">
            <button
              class="agt-header__avatar"
              :class="{ 'has-photo': agent.profilePhoto }"
            ...
            <svg
              v-if="agent.isVerified"
              class="agt-avatar-badge agt-avatar-badge--sm"
              viewBox="0 0 24 24"
              width="14"
              height="14"
            >
            ...
            <svg
              v-if="agent.isPartner"
              class="agt-partner-badge agt-partner-badge--sm"
              viewBox="0 0 24 24"
              width="14"
              height="14"
            >
              <circle cx="12" cy="12" r="11" fill="#2563eb" />
              <path fill="#fff" d="M6 12l2-2 4 2-2 2zM18 12l-2-2-4 2 2 2z" />
            </svg>
          </div>
          <div v-if="profileOpen" class="agt-profile-dropdown">
            <div class="agt-profile-dropdown__header">
              <div class="agt-avatar-wrap">
                <div
                  class="agt-profile-dropdown__avatar"
                  :class="{ 'has-photo': agent.profilePhoto }"
                >
                  <img v-if="agent.profilePhoto" :src="agent.profilePhoto" alt="" />
                  <span v-else>{{ initials }}</span>
            ...
            <router-link
              to="/agent/settings"
              class="agt-profile-dropdown__item"
    ...
    <aside class="agt-sidebar" :class="{ 'mobile-open': mobileMenuOpen }">
      ...
      <div class="agt-sidebar__profile agt-sidebar__profile--mini" v-else>
        <div class="agt-avatar-wrap">
          <div
            class="agt-sidebar__avatar agt-sidebar__avatar--sm"
            :class="{ 'has-photo': agent.profilePhoto }"
          >
            <img v-if="agent.profilePhoto" :src="agent.profilePhoto" alt="" />
            <span v-else>{{ initials }}</span>
      ...
      <nav class="agt-sidebar__nav">
        ...
        >
          <!-- Dashboard -->
          <svg v-if="item.icon === 'dashboard'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
          ...
          <!-- Content -->
          <svg v-else-if="item.icon === 'content'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
          ...
          <!-- Analytics -->
          <svg v-else-if="item.icon === 'analytics'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
          ...
          <!-- Visits -->
          <svg v-else-if="item.icon === 'visits'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
              d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-2 .9-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm0 16H5V8h14v11zM9 10H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z"
            />
          </svg>
          <!-- Wallet -->
          <svg v-else-if="item.icon === 'wallet'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
          ...
          <!-- Settings -->
          <svg v-else-if="item.icon === 'settings'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
          ...
          <!-- Support -->
          <svg v-else-if="item.icon === 'support'" viewBox="0 0 24 24" width="24" height="24">
            <path
              fill="currentColor"
    ...
    <nav v-if="isMobile" class="agt-bnav">
      ...
      <router-link
        to="/agent/listings"
        class="agt-bnav__item"
        :class="{ active: route.path.startsWith('/agent/listings') }"
      >
        <svg viewBox="0 0 24 24" width="22" height="22">
          <path
            fill="currentColor"
      ...
      <button
        class="agt-bnav__mona"
        @click="openMonaFromNav"
        aria-label="Mona - Créer une annonce vocale"
      >
      ...
      <router-link
        to="/agent/visits"
        class="agt-bnav__item"
        :class="{ active: route.path.startsWith('/agent/visits') }"
      >
        <svg viewBox="0 0 24 24" width="22" height="22">
          <path
            fill="currentColor"
...
.agt-header__mobile-search:hover {
  background: #f0f0f0;
}
...
.agt-header__avatar.has-photo {
  background: none;
}
.agt-header__avatar img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
.agt-profile-dropdown__avatar.has-photo {
  background: none;
}
.agt-profile-dropdown__avatar img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
.agt-profile-dropdown__item:hover {
  background: #f2f2f2;
}
...
.agt-sidebar__avatar.has-photo {
  background: none;
}
.agt-sidebar__avatar img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
.sidebar-collapsed .agt-sidebar__label {
  display: none;
}
.sidebar-collapsed .agt-sidebar__footer {
  display: none;
}
...
.agt-notif-panel__loading,
.agt-notif-panel__empty {
  padding: 2rem;
  text-align: center;
  color: #888;
  font-size: 0.9rem;
}
.agt-notif-panel__list {
  overflow-y: auto;
  flex: 1;
}
.agt-notif-item {
...
.agt-notif-item:hover {
  background: #f9f9f9;
}
.agt-notif-item.unread {
  background: #f0fdf4;
}
...
.agt-notif-item__icon.cat-kyc {
  background: #dcfce7;
  color: #16a34a;
}
.agt-notif-item__icon.cat-visit {
  background: #dbeafe;
  color: #2563eb;
}
.agt-notif-item__icon.cat-wallet {
  background: #fef3c7;
  color: #d97706;
}
.agt-notif-item__icon.cat-system {
  background: #f3f4f6;
  color: #6b7280;
}
.agt-notif-item__body {
  flex: 1;
  min-width: 0;
}
.agt-notif-item__title {
  font-weight: 600;
  font-size: 0.85rem;
  margin-bottom: 2px;
}
...
.agt-notif-item__time {
  font-size: 0.72rem;
  color: #999;
  margin-top: 4px;
}
.agt-notif-item__arrow {
  flex-shrink: 0;
  color: #aaa;
  transition: color 0.15s;
}
.agt-notif-item.clickable:hover .agt-notif-item__arrow {
  color: #2563eb;
}
.agt-notif-item.clickable:hover {
  background: #eef6ff;
}
.agt-notif-item {
  align-items: center;
...
.agt-bnav__mona-btn {
  ...
  animation: agt-mona-sonar 3s ease-in-out infinite;
}
@keyframes agt-mona-sonar {
  0%,
  100% {
    box-shadow:
