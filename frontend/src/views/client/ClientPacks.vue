...
const totalPhysical = computed(
  () => packs.value.filter((p) => p.has_physical_key && !p.is_locked_by_visit).length,
)

function remaining(p: Pack) {
  return Math.max(p.virtual_total - p.virtual_used, 0)
}
...
async function handleBuyPack() {
  ...
  if (typeof PaystackPop === 'undefined') {
    toast.error(
      'Le serveur de paiement Paystack est temporairement injoignable. Réessayez dans quelques minutes.',
    )
    return
  }
  ...
  try {
    ...
    posthog.capture('pack_purchase_initiated', {
      tx_ref: txRef,
      amount: data.amount,
    ...
    popup.resumeTransaction(data.access_code, {
      onSuccess: async (transaction: any) => {
        ...
        try {
          ...
          posthog.capture('pack_purchase_completed', {
            tx_ref: txRef,
            amount: data.amount,
            currency: data.currency,
            provider: data.provider,
  ...
  <div class="pk">
    ...
    <div class="pk__standby">
      ...
      <p style="margin: 4px 0 0; color: #606060; font-size: 0.9rem; text-align: center">
        Les visites virtuelles (photos &amp; vidéos) sont actuellement
        <strong>gratuites</strong> pour tous les utilisateurs connectés. Créez un compte et explorez
        tous les biens sans aucun frais.
      </p>
    </div>
    ...
    <div class="pk__hero" style="display: none">
      ...
      <h1 class="pk__hero-title">Visites <span class="pk__hero-hl">Gratuites</span></h1>
    ...
    <div class="pk__offer" style="display: none">
      ...
      <div class="pk__offer-right">
        ...
        <button class="pk__buy-btn" :disabled="buying" @click="handleBuyPack">
    ...
    <div v-else class="pk__grid">
      <div v-for="pack in packs" :key="pack.id" class="pk__card">
        <div class="pk__card-top">
          ...
          >
            {{
              pack.is_locked_by_visit
                ? 'Visite en cours'
                : remaining(pack) === 0 && !pack.has_physical_key
                  ? 'Épuisé'
                  : 'Actif'
            }}
          </span>
        ...
        <div class="pk__bar">
          <div
            class="pk__bar-fill"
            :style="{ width: (pack.virtual_used / pack.virtual_total) * 100 + '%' }"
          ></div>
        </div>
        <div class="pk__bar-label">
          {{ pack.virtual_used }}/{{ pack.virtual_total }} clés utilisées
        </div>
      </div>
    </div>
  </div>
...
.pk__buy-btn:active:not(:disabled) {
  transform: translateY(0);
}
.pk__buy-btn:disabled {
  opacity: 0.7;
  cursor: wait;
}
