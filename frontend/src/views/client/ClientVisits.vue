...
async function handleCancel() {
  ...
  try {
    await http.post(`/api/client/visits/${cancelTarget.value}/cancel/`, { reason: cancelReason.value.trim() })
...
<template>
  <div class="vis">
    ...
    <div v-else-if="filtered.length === 0" class="vis__empty">
      ...
      <router-link v-if="filter === 'ALL'" to="/home" class="vis__explore-btn">Explorer les biens</router-link>
    </div>
    ...
    <div v-else class="vis__list">
      <div v-for="visit in filtered" :key="visit.id" class="vis__card">
        ...
        <div v-if="visit.scheduled_at" class="vis__info-row">
          <svg viewBox="0 0 24 24" width="14" height="14"><path fill="#1DA53F" d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-2 .9-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm0 16H5V8h14v11z"/></svg>
          <span>Rendez-vous : <strong>{{ formatDateTime(visit.scheduled_at) }}</strong></span>
        </div>
        <div v-else-if="visit.slot_label" class="vis__info-row">
          <svg viewBox="0 0 24 24" width="14" height="14"><path fill="#606060" d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/></svg>
          <span>Créneau : {{ visit.slot_label }}</span>
        </div>

        <!-- Lieu de RDV -->
        <div v-if="visit.meeting_address || visit.meeting_map_url" class="vis__info-row">
          <svg viewBox="0 0 24 24" width="14" height="14"><path fill="#1DA53F" d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 010-5 2.5 2.5 0 010 5z"/></svg>
          <span v-if="visit.meeting_address">{{ visit.meeting_address }}</span>
          <a v-if="visit.meeting_map_url" :href="visit.meeting_map_url" target="_blank" rel="noopener" class="vis__map-link">Voir sur la carte</a>
        </div>
        ...
        <!-- Motif d'annulation -->
        <p v-if="visit.cancel_reason && visit.status === 'CANCELED'" class="vis__note vis__note--cancel">
    ...
    <div v-if="showCancelModal" class="modal-overlay" @click.self="showCancelModal = false">
      <div class="modal">
        ...
        <p v-if="cancelIsConfirmed" class="modal__warning modal__warning--danger">
          <svg viewBox="0 0 24 24" width="16" height="16"><path fill="#dc2626" d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>
          L'agent a déjà confirmé ce rendez-vous. Si vous annulez maintenant, votre <strong>clé physique ne sera pas restaurée</strong>.
        </p>
        ...
        <div class="modal__field">
          ...
          <select v-model="cancelReason" class="modal__select">
            ...
            <option value="Je ne suis plus intéressé par ce bien">Plus intéressé par ce bien</option>
            <option value="Je ne suis plus disponible à cette date">Plus disponible à cette date</option>
        ...
        <div v-if="cancelReason === 'Autre'" class="modal__field">
          ...
          <textarea v-model="cancelReason" class="modal__textarea" rows="2" placeholder="Décrivez brièvement la raison..."></textarea>
        </div>
        <div class="modal__actions">
          <button class="modal__btn modal__btn--ghost" @click="showCancelModal = false" :disabled="cancelLoading">Retour</button>
          <button class="modal__btn modal__btn--danger" @click="handleCancel" :disabled="cancelLoading || !cancelReason.trim()">
...
.vis__title { font-size: 24px; font-weight: 700; color: #0F0F0F; margin-bottom: 16px; }
...
.vis__tab.active { color: #0F0F0F; font-weight: 600; border-bottom-color: #1DA53F; }
...
.vis__empty { text-align: center; padding: 48px 16px; color: #606060; }
.vis__empty p { margin: 12px 0; font-size: 14px; }
.vis__explore-btn {
  display: inline-block; padding: 10px 24px; background: #1DA53F;
  color: #fff; border-radius: 20px; text-decoration: none; font-weight: 600;
...
.vis__status { padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 600; }
.st--pending { background: #fef3c7; color: #d97706; }
.st--info { background: #dbeafe; color: #2563eb; }
.st--success { background: rgba(29,165,63,.1); color: #1DA53F; }
.st--danger { background: #fef2f2; color: #dc2626; }
...
.vis__meta { display: flex; align-items: center; gap: 4px; font-size: 13px; color: #606060; margin-bottom: 8px; }

.vis__info-row {
  display: flex; align-items: center; gap: 6px;
  font-size: 13px; color: #272727; margin-bottom: 4px;
}
.vis__map-link { color: #1DA53F; text-decoration: none; font-weight: 500; font-size: 13px; }
.vis__map-link:hover { text-decoration: underline; }

.vis__note {
  font-size: 13px; color: #606060; margin-top: 8px;
  padding: 8px 12px; background: #f8f8f8; border-radius: 8px;
}
.vis__note--agent { background: #eff6ff; color: #1e40af; }
.vis__note--cancel { background: #fef2f2; color: #991b1b; }

.vis__code {
  background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 10px;
  padding: 14px 18px; margin: 10px 0; text-align: center;
}
.vis__code-label { font-size: 12px; color: #606060; margin-bottom: 4px; }
.vis__code-value { font-size: 28px; font-weight: 700; letter-spacing: 6px; color: #1DA53F; font-family: monospace; }
.vis__code-hint { font-size: 11px; color: #888; margin-top: 6px; }

.vis__actions { display: flex; gap: 8px; margin-top: 12px; }
.vis__cancel-btn {
  padding: 8px 18px; border: 1px solid #dc2626; background: transparent;
  color: #dc2626; border-radius: 20px; font-size: 13px; font-weight: 500;
  cursor: pointer; transition: background .15s;
}
.vis__cancel-btn:hover { background: rgba(220,38,38,.06); }
...
.modal__title { font-size: 20px; font-weight: 700; color: #0F0F0F; margin-bottom: 12px; }
.modal__warning {
...
.modal__warning svg { flex-shrink: 0; margin-top: 1px; }
.modal__warning--danger { color: #991b1b; background: #fef2f2; border-color: #fecaca; }
.modal__field { margin-bottom: 16px; }
.modal__field label { display: block; font-size: 14px; font-weight: 500; color: #272727; margin-bottom: 6px; }
.req { color: #dc2626; }
.modal__select {
  width: 100%; padding: 10px 12px; border: 1px solid #E0E0E0;
  border-radius: 8px; font-size: 14px; color: #0F0F0F; background: #fff;
}
.modal__select:focus { outline: none; border-color: #1DA53F; }
.modal__textarea {
  width: 100%; padding: 10px 12px; border: 1px solid #E0E0E0;
  border-radius: 8px; font-size: 14px; color: #0F0F0F;
  resize: vertical; font-family: inherit; box-sizing: border-box;
}
.modal__textarea:focus { outline: none; border-color: #1DA53F; }
...
.modal__btn--ghost { background: #f2f2f2; color: #272727; }
.modal__btn--ghost:hover:not(:disabled) { background: #e5e5e5; }
.modal__btn--danger { background: #dc2626; color: #fff; }
.modal__btn--danger:hover:not(:disabled) { background: #b91c1c; }
</style>
